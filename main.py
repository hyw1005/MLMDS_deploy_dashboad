...
# 날짜를 비교하기 위한 함수(start와 end의 날짜를 비교하기 위해 사용)
...
with col1:
    with st.form("form_left"):
        st.subheader("🍎 Left Form")
        name1 = st.text_input("Your name (left)")
        age1 = st.number_input("Your age (left)", min_value=0)
        submitted1 = st.form_submit_button("Submit Left")
        if submitted1:
            st.success(f"👈 Left Form submitted: {name1}, {age1} years old")

with col2:
    with st.form("form_right"):
        st.subheader("🍊 Right Form")
        name2 = st.text_input("Your name (right)")
        age2 = st.number_input("Your age (right)", min_value=0)
        submitted2 = st.form_submit_button("Submit Right")
        if submitted2:
            st.success(f"👉 Right Form submitted: {name2}, {age2} years old")   